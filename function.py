...
def generate_code(passwd_length, batch_size, device, inv, gen_random_WR):
    ...
    # print(f"循环After first loop, p: {p}")        
    ...
    if not inv:
        ...
        SCALE = 0.3  # 控制 embedding 影响力，避免 modulation 过强
        if gen_random_WR:
            # print(f"genWR1 p: {(p-0.5) * SCALE}")
            # print(f"genWR1 rand_p: {(rand_p-0.5) * SCALE}")
            # print(f"genWR1 rand_inv_p: {(rand_p-0.5) * SCALE}")
            return (p-0.5) * SCALE, (rand_p-0.5) * SCALE, (inv_p-0.5) * SCALE, (rand_inv_p-0.5) * SCALE, (rand_inv_2nd_p-0.5) * SCALE
        # print(f"genWR2 p: {(p-0.5) * SCALE}")
        return (p-0.5) * SCALE, (rand_p-0.5) * SCALE, (inv_p-0.5) * SCALE, (rand_inv_p-0.5) * SCALE
